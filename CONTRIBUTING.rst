...
::
    ...
    $ git clone git@github.com:jofsky/MetaPush
    $ cd MetaPush

Making a virtual env
--------------------

Project Name development requires Python v3.7. If ``which python3.7`` returns a
...
create a virtualenv (once you have `virtualenvwrapper`_ installed locally)::
    ...
    $ mkvirtualenv MetaPush --python=$(which python3.7)
...
Now run ``workon MetaPush`` again to set those environment variables.

Your ``PATH`` (and environment variables) will be updated when you ``workon
MetaPush`` and restored when you ``deactivate``. This will make sure that
